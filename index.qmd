...
---
```{=html}
<h1 style="text-align:center">Probability Theorem and Random variable</h1> 
```
...
x_values = np.linspace(df[column_of_interest].min(), df[column_of_interest].max(), 100)
...
columns = df.columns
fig, axes = plt.subplots(nrows=len(columns), ncols=1, figsize=(12, 8 * len(columns)))
...
sns.lineplot(x='Year', y='Episodes', data=df, estimator='sum', ci=None, marker='o', color='blue')
plt.xticks(rotation = 90,
           fontsize = 10)
plt.xlim(0, 178)
plt.title('TV Show Production Trends Over the Years')
plt.xlabel('Release Year')
plt.ylabel('Total Number of Episodes')
plt.show()
```
```{python}
...
```{=html}
<h1 style="text-align:center">Linear Regression
</h1> 
```
```{python}
...
plt.title('Count of Cylinder Number')
...
```{=html}
<h1 style="text-align:center">Non-Linear Regression</h1>
```
```{python}
...
cm_lgb = confusion_matrix(y_test, ypred_lgb)
...
```{=html}
<h1 style="text-align:center">Outlier Detection</h1>
```
```{python}
import numpy as np
import pandas as pd
...
for i, feature1 in enumerate(features, 1):
    for j, feature2 in enumerate(features, 1):
        plt.subplot(len(features), len(features), (i - 1) * len(features) + j)
